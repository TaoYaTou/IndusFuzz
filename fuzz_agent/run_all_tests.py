...
# 如果脚本在 IndusFuzz\ 根目录调用，_SCRIPT_DIR 就是根
# 如果脚本在 IndusFuzz\fuzz_agent\ 调用，_SCRIPT_DIR 就是 fuzz_agent
# 我们约定：ROOT = IndusFuzz 根，FUZZ = fuzz_agent 子目录
if os.path.basename(_SCRIPT_DIR) == "fuzz_agent":
    FUZZ_ROOT = _SCRIPT_DIR
    ROOT = os.path.dirname(_SCRIPT_DIR)
else:
    ROOT = _SCRIPT_DIR
    FUZZ_ROOT = os.path.join(_SCRIPT_DIR, "fuzz_agent")

REPORT_DIR = os.path.join(ROOT, "test")
os.makedirs(REPORT_DIR, exist_ok=True)

sys.path.insert(0, FUZZ_ROOT)
