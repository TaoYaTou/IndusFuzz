...
### P1-3 开发完成后 MD 文件同步（**强制，防 L11/L15/L16**）
每次开发完成（含新协议、改业务逻辑、修 bug），**必须检查并同步更新**以下 6 份 MD（缺一份 = L11 违规）：

| MD 文件 | 更新触发 | 更新内容 |
|---------|---------|---------|
| **CHANGELOG.md** | 每次有功能/bugfix | 新增版本条目，按本次改动填写 |
| **docs/IndusFuzz 协议扩展 PRD.md** | 新增协议 | 追加协议模板实例，把 L17/L18/L20 教训写进去 |
| **docs/IndusFuzz 审计PRD.md** | 审计发现新问题 | 追加新审计维度（防 L11） |
| **docs/IndusFuzz 审计修复总结PRD.md** | 本轮审计修复 | 追加修复总结 |
| **RULES_RELEASE.md 教训表** | 踩到新坑 | 追加新教训编号 L{n} |
| **README.md / README.zh.md** | 新增协议或重大功能 | 更新协议列表、快速开始、截图 |
...
4. **外部触发审计的文档**（`docs/IndusFuzz 审计PRD.md` 等）— 同步更新审计维度描述
