...
每次开发完成（含新协议、改业务逻辑、修 bug），**必须按以下优先级同步 8 份文档**（缺一份 = L11 违规）：
...
**审计执行顺序**：先按 `docs/IndusFuzz 审计PRD.md` 的章节 5.12 / 5.14 / 5.15 逐条跑 `audit_prd.py`，发现问题后查 `docs/IndusFuzz 审计修复总结PRD.md` 是否已有同类修复记录（防重复踩坑），然后按上述 4 处同步。
